    ...
    public class KraRoadmapCoreSupportProcessRepository : BaseRepository<KraRoadmapCoreSupportProcess, int, ImisDbContext, User>, IKraRoadmapCoreSupportProcessRepository
    {
        ...
        public async Task<List<KraProcessGroupedDto>> GetGroupedAsync(int kraRoadMapPeriodId, CancellationToken cancellationToken)
        {
            var groupedData = await ReadOnlyDbContext.Set<KraRoadmapProcessKraAssignment>()
                ...
                .Where(x => !x.IsDeleted)
                .Join(
                    ReadOnlyDbContext.Set<KeyResultArea>(),
                    a => a.KraId,
                    k => k.Id,
            ...
            var kraIds = groupedData.SelectMany(x => x.Kras.Select(k => k.Id)).Distinct().ToList();

            var kraRoadmaps = await ReadOnlyDbContext.Set<KraRoadMap>()
                .AsNoTracking()
                .Where(r => !r.IsDeleted && r.KraId.HasValue && kraIds.Contains(r.KraId.Value) && r.KraRoadMapPeriodId == kraRoadMapPeriodId)
                .Select(r => new
                {
                    KraId = r.KraId!.Value,
                    RawKpis = r.Kpis!
            ...
            var allKpiIds = kraRoadmaps.SelectMany(x => x.RawKpis.Select(k => (long)k.Id)).Distinct().ToList();

            var strategyReviewActuals = await ReadOnlyDbContext.Set<StrategyReviewDeliverableKpi>()
                .AsNoTracking()
                .Where(x => !x.IsDeleted)
                .Join(
                    ReadOnlyDbContext.Set<StrategyReview>().Where(sr => !sr.IsDeleted),
            ...
            foreach (var process in groupedData)
            {
                ...
                {
                    var roadmap = kraRoadmaps.FirstOrDefault(x => x.KraId == kra.Id);
                    ...
                    kra.Kpis = roadmap.RawKpis.GroupBy(x => (x.KpiDescription ?? "").Trim()).Select(group =>
                    {
                        var first = group.OrderBy(x => x.Year ?? int.MaxValue).First();
                        ...
                        var yearMap = group.Where(x => x.Year.HasValue).ToDictionary(x => x.Year!.Value);

                        foreach (var year in roadmap.DeliverableYears)
                        {
                            yearMap.TryGetValue(year, out var row);

                            dto.Targets.Add(FormatValue(row?.Target));
